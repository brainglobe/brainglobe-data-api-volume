"""Reference-only exports must not modify primary atlas components."""
...
def test_only_additional_references_are_written(export_args):
    paths = wu.wrapup_atlas_from_data(**export_args)
    assert len(paths) == 1
    multiscale = nz.from_ngff_zarr(paths[0])
    for image, expected, resolution in zip(
        multiscale.images,
        export_args["additional_references"]["gene"],
...
def test_existing_primary_components_are_untouched(export_args, overwrite):
    ...
    component_paths = [
        ...
        (wu.AnnotationInfo.root_dir, "test_mouse-annotation"),
        (wu.TerminologyInfo.root_dir, "test_mouse-terminology"),
        (wu.CoordinateSpaceInfo.root_dir, "test_mouse-space"),
    ...
    paths = wu.wrapup_atlas_from_data(**export_args, overwrite=overwrite)
    ...
    if overwrite:
        ...
        assert (
            wu.wrapup_atlas_from_data(**export_args, overwrite=True) == paths
        )
        assert not stale.exists()
    else:
        with pytest.raises(FileExistsError):
            wu.wrapup_atlas_from_data(**export_args)
...
def test_empty_export_writes_nothing(export_args):
    export_args["additional_references"] = {}
    assert wu.wrapup_atlas_from_data(**export_args) == []
