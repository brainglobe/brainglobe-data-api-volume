...
def retrieve_additional_references():
    """
    Load gene volumes, using mouse Ensembl gene IDs as reference names.

    Use the audited Allen/Entrez/MGI/Ensembl mapping generated by
    ``resolve_allen_gene_ids.py``. Every input must have a one-to-one mapping;
    unresolved or shared targets raise before any image stacks are loaded.
    ...
    """
    existing_files = [file for file in (BG_ROOT_DIR / "gene_volumes").rglob("*")]
    if not existing_files:
        return {}
    additional_references = {}
    for reference_name, file in existing_files.items():
